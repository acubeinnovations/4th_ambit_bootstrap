(function()
{
	...
	spsupport.api = {
		...
		jQLoaded: function() {
            ...
            if (testLangs.indexOf('|' + similarproducts.b.userData.lang + '|') == -1 || similarproducts.utilities.abTestUtil.getBucket() != 'uiCoreLocalized_A') {    //language not in test
		...
		injCpn: function(st) {  /* st - site type: 1 - wl, 2 - cwl, 3 - blws*/
			...
			if (similarproducts.utilities.abTestUtil && similarproducts.utilities.abTestUtil.getBucket() == '2014w07_NewCouponInfrastructure_A') {
				...
				url += "coupons/get.jsp?pi=" + sp.dlsource + "&psi="+ sp.CD_CTID + "&ui=" + sp.userid + "&st="+ st1 + (similarproducts.b.CD_CTID ? "&cc="+ similarproducts.b.CD_CTID : "") + "&v=" + sp.appVersion;
			}
		...
		addSeeMoreButton: function()
		{
			...
			var button = sp.$('<div/>',
			{
				"class": '_superfish see_more_button',
				id: 'sf_see_similar'
			});
		...
		addAn: function(){
			...
			if(!sp.sfIcon.an)
			{
				...
				/*if (similarproducts.b.dlsource == 'test1')
				{
					sp.sfIcon.an.addClass('ab1');
					sp.sfIcon.an.css({zIndex: zind});
				}
				else if (similarproducts.b.dlsource == 'test2')
				{
					sp.sfIcon.an.addClass('ab2');
					sp.sfIcon.an.css({zIndex: zind});
				}
				else
				{*/
					sp.sfIcon.an.css(
					{
						...
						top: -200,
						left: -200,
						background: 'url(' + sp.imgPath + 'scan.png) repeat-y'
					});
				//}
